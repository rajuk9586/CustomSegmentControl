...
class CustomSegmentControl5: UIControl {
    var buttons: [UIButton] = []
    var selector: UIView!
    var selectedSegmentIndex = 0
    private var isViewUpdated = false
    ...
    func updateView() {
        ...
        let buttonImages = commaSeparatedButtonTitles.components(separatedBy: ",")
        ...
        buttons[selectedSegmentIndex].imageView?.tintColor = selectorTextColor
        
        let selectorWidth = frame.height
        let selectorStartPosition = frame.width / CGFloat(buttons.count) * CGFloat(selectedSegmentIndex)
        selector = UIView(frame: CGRect(x: selectorStartPosition + 2, y: 2, width: selectorWidth - 4, height: frame.height - 8))
        selector.layer.cornerRadius = selector.frame.height / 2
        selector.layer.masksToBounds = true
        self.selector.backgroundColor = self.selectorColor
        addSubview(selector)
